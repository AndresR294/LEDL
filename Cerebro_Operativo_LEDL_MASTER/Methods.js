...
module.exports = function(God) {
  ...
  God.processIsDead = function(pid, pm2_env, cb, sigkill) {
    ...
    var treePids     = pm2_env._tree_pids || [pid];

    function allDead() {
      for (var i = 0; i < treePids.length; i++) {
        if (God.checkProcess(treePids[i]) !== false)
          return false;
      }
      return true;
    }
    ...
    timeout = setTimeout(function() {
      ...
      if (sigkill) {
        ...
        var alive = survivors();
        console.log('Process with pid %d still alive after %sms, sending SIGKILL to %d remaining pids...', pid, kill_timeout, alive.length);
